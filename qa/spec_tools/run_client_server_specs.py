...
LATEST_REPORT = REPORT_DIR / "latest-client-server-spec-report.md"
...
def _collect_suggestions(output: str, exit_code: int) -> list[tuple[str, str, str]]:
    ...
    if exit_code == 0:
        return [
            (
                "MAINTAIN_CLIENT_SERVER_SPEC_COVERAGE",
                "All client-server spec tests are passing",
                "Add additional cross-layer spec cases for new behavior and rerun the pipeline.",
            )
        ]
    if "Connection refused" in output and ":5173" in output:
        suggestions.append(
            (
                "START_FRONTEND",
                "Start frontend dev server and rerun client-server specs",
    ...
    if "Connection refused" in output and ":8000" in output:
        suggestions.append(
            (
                "START_BACKEND",
                "Start backend server and rerun client-server specs",
    ...
    if "Unexpected HTTP status" in output:
        suggestions.append(
            (
                "CHECK_CLIENT_SERVER_ROUTE_OR_CONTRACT",
                "Review failing path and expectation for client-server specs",
                "Compare `spec/testing/client-server/*.md` with current UI/API behavior.",
            )
        )
    if not suggestions:
        suggestions.append(
...
def _write_report(
    ...
    lines: list[str] = []
    lines.append("# Client-Server Specification Test Report")
    lines.append("")
    lines.append(f"- Generated at: `{now}`")
    lines.append(f"- Exit code: `{exit_code}`")
    lines.append(f"- Client-server spec cases: `{generated_summary['client_server']}`")
