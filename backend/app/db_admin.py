...
MODULE_ROOT = (__package__ or "app").strip()
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default="refresh",
        help="recreate=drop/create+seed, migrate=schema verify/create-missing-tables only (no ALTER), refresh=migrate+clean+seed, clean=data only, migrate-audit-fields=add/backfill CREATED_BY from CHANGED_BY, migrate-medical-value-units=add/backfill LOINC+UCUM medical value columns, verify-medical-value-units=read-only LOINC+UCUM coverage verification, migrate-procurement-runtime=legacy->unified procurement backfill, migrate-procurement-typed=unified->typed procurement backfill, clear-translation-bundles=delete DB translation overrides, normalize-legacy-dev-forum-capture-label=normalize stale Dev-Forum capture label overrides",
    )
    ...
    def run(module_suffix: str, module_args: list[str]) -> int:
        module = f"{MODULE_ROOT}.{module_suffix}"
    ...
    def run_with_dev_forum_backup(
        *,
        ...
        export_code = run("db_data", ["--mode", "export-dev-forum", "--env", args.env, *export_args, *db_url_args])
        if export_code != 0:
            return export_code

        schema_code = run("db_schema", ["--mode", schema_mode, "--env", args.env, *db_url_args])
        if schema_code != 0 and data_mode == "refresh" and schema_mode == "migrate":
            # refresh fallback: recreate when migrate fails
            schema_code = run("db_schema", ["--mode", "recreate", "--env", args.env, *db_url_args])
        if schema_code != 0:
            return schema_code

        data_code = run("db_data", ["--mode", data_mode, "--env", args.env, *data_args, *db_url_args])
        if data_code != 0:
            return data_code

        import_code = run("db_data", ["--mode", "import-dev-forum", "--env", args.env, *export_args, *db_url_args])
        if import_code != 0:
            print(
    ...
    if args.mode == "migrate":
        return run("db_schema", ["--mode", "migrate", "--env", args.env, *db_url_args])

    if args.mode == "clean":
        return run("db_data", ["--mode", "clean", "--env", args.env, *db_url_args])
    ...
    if args.mode == "clear-translation-bundles":
        return run("db_data", ["--mode", "clear-translation-bundles", "--env", args.env, *db_url_args])
